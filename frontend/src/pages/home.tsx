...
export default function Home() {
  const colorTheme = {
    ...
    darkMid: "rgba(0, 0, 0, 0.6)",
    darkBack: "rgba(0, 0, 0, 0.1)",
    lightMain: "rgba(255, 255, 255, 1)",
    lightMid: "rgba(255, 255, 255, 0.6)",
  ...
  return (
    <div className="w-full h-full relative">
      ...
      >
        ...
        >
          <div
            className={`absolute -top-[2px] right-0 flex items-center rounded-full border-2 transition-all`}
            style={{
          ...
          >
            {darkMode ? (
              <Moon size={32} style={{ stroke: `${mainColor}` }} />
            ) : (
              <Sun size={32} style={{ stroke: `${mainColor}` }} />
            )}
          </div>
        </button>
      </nav>
